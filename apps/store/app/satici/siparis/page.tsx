...
const DURUM: Record<ShipmentStatus, { bg: string; fg: string; text: string }> = {
...
const KARGO_FIRMALARI = ["Yurtiçi Kargo", "Aras Kargo", "MNG Kargo", "PTT Kargo", "Sürat Kargo", "UPS", "DHL"];
...
async function siparisApi<T>(path: string, token: string): Promise<T | null> {
  try {
...
export default async function SaticiSiparisler({
  searchParams,
}: {
  ...
  async function kargoIslem(formData: FormData) {
    "use server";
    const s = await auth();
    const t = (s as unknown as { accessToken?: string } | null)?.accessToken;
    if (!t) return;
    const id = String(formData.get("id"));
    const islem = String(formData.get("islem")); // ship | deliver | cancel
    const govde =
      islem === "ship"
        ? {
    ...
    try {
      res = await fetch(`${purchaseApi()}/api/seller/orders/${id}/${islem}`, {
        method: "POST",
    ...
    const donus = String(formData.get("donus") ?? "/satici/siparis");
    revalidatePath("/satici/siparis");
    const ek = donus.includes("?") ? "&" : "?";
    // redirect() try/catch dışında — NEXT_REDIRECT yutulmasın.
    if (!res || !res.ok) {
      let mesaj = res ? `HTTP ${res.status}` : "Sunucuya ulaşılamadı";
      if (res) {
        try {
          mesaj = (await res.json()).message ?? mesaj;
        } catch { /* gövde yok */ }
      }
      redirect(`${donus}${ek}hata=${encodeURIComponent(mesaj)}`);
    }
    redirect(`${donus}${ek}ok=1`);
  }

  const back = `/satici/siparis${durum ? `?durum=${durum}` : ""}`;
  ...
  return (
    <div style={{ maxWidth: 900, display: "grid", gap: 18 }}>
      ...
      <section style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(150px, 1fr))", gap: 12 }}>
        {[
          { l: "Toplam sipariş", v: String(ozet?.orderCount ?? 0) },
          { l: "Toplam satış", v: tl(Number(ozet?.totalRevenue ?? 0)) },
          { l: "Kargo bekleyen", v: String(ozet?.awaitingShipment ?? 0) },
        ].map((k) => (
      ...
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
        {SEKMELER.map((t) => (
          <a key={t.key} href={t.key ? `/satici/siparis?durum=${t.key}` : "/satici/siparis"}
      ...
      <section>
        <h2 style={{ fontSize: 17 }}>Siparişler ({liste.length})</h2>
        {liste.length === 0 ? (
          <p style={{ color: "var(--gg-muted)" }}>
            {durum ? "Bu durumda sipariş yok." : "Henüz sipariş yok."}
          </p>
        ) : (
          <div style={{ display: "grid", gap: 12 }}>
            {liste.map((o) => {
              const d = o.shipmentStatus ? DURUM[o.shipmentStatus] : null;
              return (
                <article key={o.id} className="gg-card" style={{ display: "grid", gap: 10 }}>
                  <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
                    <strong>{urunAdi.get(o.productId) ?? "Ürün"}</strong>
                    {d ? (
                      <span style={{
                  ...
                  {o.trackingNumber ? (
                    <div style={{
                      ...
                      {o.shippedAt ? ` · Kargoya veriliş: ${tarih(o.shippedAt)}` : ""}
                      {o.deliveredAt ? ` · Teslim: ${tarih(o.deliveredAt)}` : ""}
                    </div>
                  ) : null}
                  ...
                  {o.shipmentStatus === "HAZIRLANIYOR" ? (
                    <div style={{ display: "grid", gap: 8 }}>
                      <form action={kargoIslem} style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                        <input type="hidden" name="id" value={o.id} />
                        <input type="hidden" name="islem" value="ship" />
                      ...
                      <form action={kargoIslem}>
                        <input type="hidden" name="id" value={o.id} />
                        <input type="hidden" name="islem" value="cancel" />
                        <input type="hidden" name="donus" value={back} />
                    ...
                    <form action={kargoIslem}>
                      <input type="hidden" name="id" value={o.id} />
                      <input type="hidden" name="islem" value="deliver" />
                      <input type="hidden" name="donus" value={back} />
