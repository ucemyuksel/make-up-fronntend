...
export default async function SellerNotifications({
  ...
  const { token } = await requireSeller("/seller/notifications");

  const [notifications, sohbetler] = await Promise.all([
    getir<Notification[]>(NOTIF_API(), "/api/notifications", token),
    getir<Conversation[]>(MSG_API(), "/api/conversations", token),
  ]);
  const list = notifications ?? [];
  const unread = list.filter((n) => !n.read).length;
  const conversationList = sohbetler ?? [];
  const unreadMessages = conversationList.reduce((t, c) => t + Number(c.unread ?? 0), 0);
  ...
  async function markAllRead() {
    ...
    redirect("/seller/notifications?ok=1");
  }
  ...
  return (
    <div style={{ maxWidth: 860, display: "grid", gap: 18 }}>
      ...
      <section style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: 12 }}>
        {[
          { l: "Okunmamış bildirim", v: String(unread) },
          { l: "Okunmamış mesaj", v: String(unreadMessages) },
          { l: "Aktif sohbet", v: String(conversationList.length) },
        ].map((k) => (
      ...
      <section style={{ display: "grid", gap: 10 }}>
        ...
        {conversationList.length === 0 ? (
          ...
          <div style={{ display: "grid", gap: 8 }}>
            {conversationList.slice(0, 6).map((c) => (
              <a key={c.id} href={`${SOCIAL}/messages?c=${c.id}`} className="gg-card"
                ...
                <span style={{ fontSize: 11.5, color: "var(--gg-muted)" }}>{time(c.lastMessageAt)}</span>
