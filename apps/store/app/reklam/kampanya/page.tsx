...
export default async function AdCreate({ searchParams }: { searchParams: { error?: string } }) {
  ...
  return (
    <div style={{ maxWidth: 680, display: "grid", gap: 16 }}>
      ...
      <form action={olustur} className="gg-card" style={{ display: "grid", gap: 16 }}>
        <label style={lbl}>
          Reklam adı
        ...
        <div style={bolum}>
          ...
          <label style={lbl}>
            ...
            <input id="reklam-medya" name="mediaUrl" required className="gg-search"
                   placeholder="https://... (yükleyince otomatik dolar)" />
          </label>
          <MediaUpload targetId="reklam-medya" label="📤 Görsel yükle" accept="image/*,video/*" />
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <label style={lbl}>
              Medya türü
