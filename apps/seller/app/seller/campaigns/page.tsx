...
export default async function CampaignForm({ searchParams }: { searchParams: { store?: string; ok?: string; error?: string } }) {
  ...
  async function addCampaign(formData: FormData) {
    ...
    if (!t) return;
    const toIsoDate = (v: FormDataEntryValue | null) => (v ? new Date(String(v)).toISOString() : null);
    ...
    redirect(r.ok ? `/seller/campaigns?store=${store}&ok=1` : `/seller/campaigns?store=${store}&error=${encodeURIComponent(r.error ?? "error")}`);
  }
  ...
  return (
    <div style={{ maxWidth: 620, display: "grid", gap: 16 }}>
      ...
      <form action={addCampaign} className="gg-card" style={{ display: "grid", gap: 12 }}>
      ...
      <section>
        <SectionHeader title={`Mevcut Campaigns (${campaigns.length})`} small />
            ...
            <div key={c.id} className="gg-card" style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10 }}>
              <div>
                ...
                <div style={{ fontSize: 12.5, color: "var(--gg-muted)" }}>
                  {c.discountType === "PERCENT" ? `%${c.discountValue} discount` : `₺${c.discountValue} discount`}
                  {" · "}
