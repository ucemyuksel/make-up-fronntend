...
export default async function CacheYonetim({ searchParams }: { searchParams: { ok?: string; error?: string } }) {
  ...
  const { token } = await requireSeller("/seller/cache");
  ...
  async function clearCache(formData: FormData) {
    ...
    revalidatePath("/seller/cache");
    redirect(ok ? `/seller/cache?ok=${purged}` : `/seller/cache?error=1`);
  }
  ...
  return (
    <div style={{ maxWidth: 760, display: "grid", gap: 20 }}>
      ...
      <div style={{ background: "var(--gg-primary-soft)", color: "var(--gg-primary-dark)", borderRadius: 12, padding: "14px 16px", fontSize: 13.5, lineHeight: 1.5 }}>
        Ürünler hız için <strong>önbelleğe</strong> alınır (varsayılan 120 sn). Fiyat/stock
      ...
      <section>
        ...
        <div style={{ display: "grid", gap: 12 }}>
          ...
          {stores.length === 0 ? <p style={{ color: "var(--gg-muted)" }}>Mağazanız yok. <a href="/seller" className="gg-see-all">Panelden açın ›</a></p> : null}
        </div>
      </section>
    </div>
  );
