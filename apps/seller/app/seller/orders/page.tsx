...
export default async function SellerOrders({
  ...
  const { token } = await requireSeller("/seller/orders");
  ...
  async function shipmentAction(formData: FormData) {
    ...
    const returnTo = String(formData.get("returnTo") ?? "/seller/orders");
  ...
  const back = `/seller/orders${status ? `?status=${status}` : ""}`;
  ...
  return (
    <div style={{ maxWidth: 900, display: "grid", gap: 18 }}>
      ...
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
        {TABS.map((t) => (
          <a key={t.key} href={t.key ? `/seller/orders?status=${t.key}` : "/seller/orders"}
