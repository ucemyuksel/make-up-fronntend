...
export default async function SellerPanel({ searchParams }: { searchParams: { ok?: string; error?: string } }) {
  ...
  async function openStore(formData: FormData) {
    "use server";
    const s = await auth();
    ...
    if (!r.ok) redirect(`/seller?error=${encodeURIComponent(r.error ?? "error")}`);
  }
  ...
  return (
    <div style={{ maxWidth: 860, display: "grid", gap: 22 }}>
      ...
      {/* Mağaza aç */}
      <form action={openStore} className="gg-card" style={{ display: "grid", gap: 10 }}>
        ...
        <div style={{ display: "grid", gap: 12 }}>
          {stores.map((st) => (
            <div key={st.id} className="gg-card" style={{ display: "flex", gap: 12, alignItems: "center", flexWrap: "wrap" }}>
              ...
              <a href={`/seller/products?store=${st.id}`} className="gg-btn gg-btn-primary">Ürün Ekle</a>
              <a href={`/seller/campaigns?store=${st.id}`} className="gg-btn gg-btn-ghost">Kampanya</a>
              <a href="/seller/cache" className="gg-btn gg-btn-ghost">🧹 Cache</a>
            </div>
          ))}
