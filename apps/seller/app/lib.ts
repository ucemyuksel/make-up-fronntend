...
// ---------------------------------------------------------------------------
// purchase-service istemcisi (sipariş, kargo, iade)
//
// Ayrı yardımcı: `api`/`send` store-service'e gider. İkisini tek fonksiyonda
// birleştirmek her çağrıda "hangi servis" parametresi taşımak demekti ve
// yanlış servise gitmek sessizce 404 üretirdi.
// ---------------------------------------------------------------------------

const purchaseBase = () => process.env.PURCHASE_API ?? "http://localhost:8088";

export async function orderApi<T>(path: string, token: string): Promise<T | null> {
  try {
    const res = await fetch(`${purchaseBase()}${path}`, {
      headers: { Authorization: `Bearer ${token}` },
      cache: "no-store",
    });
    return res.ok ? ((await res.json()) as T) : null;
  } catch {
    return null; // servis erişilemez → sayfayı çökertme
  }
}

/** purchase-service'e yazma isteği. Hata metni backend'den aynen taşınır. */
export async function orderSend(
  ...
  let res: Response;
  try {
    res = await fetch(`${purchaseBase()}${path}`, {
      method,
...
/** Sipariş anındaki teslimat adresi. Kişisel veri — yalnız detayda döner. */
...
/** Ürün sorusu. Ürün sayfasında askerUserId gizlenir; panelde dolu gelir. */
...
/** Kupon: indirim KURALI. Bakiyesi yoktur; şartlar tutarsa uygulanır. */
export type Coupon = {
  id: string;
  storeId: string;
  code: string;
  ...
  /** Yüzde indirimde üst sınır; null = sınırsız. */
  maxDiscountAmount: number | null;
  minOrderAmount: number;
  currency: string;
  startsAt: string | null;
  endsAt: string | null;
  /** null = toplam kullanım sınırsız. */
  maxUses: number | null;
  usedCount: number;
  perUserLimit: number;
  active: boolean;
  /** Boş liste = kupon mağazanın tüm ürünlerinde geçerli. */
  productIds: string[];
};

/** Hediye kartı: PARA taşır. Kısmen harcanır, biter. */
export type GiftCard = {
  id: string;
  storeId: string;
  /** Yalnız basıldığı yanıtta dolu; listede her zaman null. */
  code: string | null;
  codeLast4: string;
...
/**
 * Yazma isteği (satıcı ekranları). Durum, hata mesajı ve yanıt gövdesi döner.
 *
 * `data` gerekiyor çünkü bazı yanıtlar <b>bir kez</b> dönen bilgi taşıyor:
 * hediye kartının ham kodu saklanmıyor ve yalnız basıldığı yanıtta var.
 * Gövdeyi atarsak o kod kaybolur.
 */
export async function send(
  ...
  try {
    res = await fetch(`${process.env.STORE_API}${path}`, {
      method,
      headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" },
      // DELETE govdesiz gider: bazi vekiller govdeli DELETE isteklerini duserir.
      body: body === undefined ? undefined : JSON.stringify(body),
      cache: "no-store",
    });
  } catch {
  ...
  if (res.ok) {
    ...
    try {
      ...
      data = undefined; // 204 gibi gövdesiz yanıtlar
    }
    return { ok: true, status: res.status, data };
  }
  let error = `HTTP ${res.status}`;
