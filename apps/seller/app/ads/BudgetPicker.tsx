...
export function BudgetPicker() {
  ...
  const dailyImpressions = cpm > 0 ? Math.round((butce / cpm) * 1000) : 0;
  ...
  return (
    <div style={{ display: "grid", gap: 12 }}>
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
        {PAKETLER.map((p) => {
          const selected = butce === p.butce && cpm === p.cpm;
          return (
            <button
              key={p.ad}
              type="button"
              className={`gg-btn ${selected ? "gg-btn-primary" : "gg-btn-ghost"}`}
      ...
      <div style={{
        ...
        📊 Bu bütçeyle günde yaklaşık <strong>{dailyImpressions.toLocaleString("tr-TR")} gösterim</strong>,
        aylık üst sınır <strong>{tl(aylik)}</strong>. Gerçek rakam rekabete ve hedef bölgeye göre değişir;
        günlük bütçe dolunca ad otomatik durur, aşım olmaz.
      </div>
    </div>
  );
}
