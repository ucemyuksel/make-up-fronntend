...
export default async function Content({
  ...
  searchParams: { ok?: string; error?: string; filter?: string; kind?: string };
}) {
  const session = (await auth()) as { accessToken?: string; roles?: string[] } | null;
  if (!session?.accessToken) redirect("/");
  if (!session.roles?.includes("ADMIN")) redirect("/forbidden");

  const isReels = searchParams.kind === "video";
  const onlyDecided = searchParams.filter === "decided";
  ...
  // Sekme + filter durumunu koruyan geri dönüş adresi.
  const back = `/content?${new URLSearchParams({
    ...(isReels ? { kind: "video" } : {}),
    ...(onlyDecided ? { filter: "decided" } : {}),
  })}`;
  ...
  async function moderate(form: FormData) {
    ...
    const returnTo = String(form.get("returnTo") ?? "/content");
  ...
  return (
    <main style={{ maxWidth: 980, margin: "0 auto", padding: 32, display: "grid", gap: 22 }}>
      ...
      <div style={{ display: "flex", gap: 8, borderBottom: "1px solid var(--gg-border, #EEE)", paddingBottom: 10 }}>
        <a href={`/content${onlyDecided ? "?filter=kararli" : ""}`}
           className={`gg-btn ${isReels ? "gg-btn-ghost" : "gg-btn-primary"}`}>
          📝 Gönderiler
        </a>
        <a href={`/content?kind=video${onlyDecided ? "&filter=kararli" : ""}`}
      ...
      <div style={{ display: "flex", gap: 8 }}>
        <a href={isReels ? "/content?kind=video" : "/content"}
           className={`gg-btn ${onlyDecided ? "gg-btn-ghost" : "gg-btn-primary"}`}>
          Tüm içerik
        </a>
        <a href={isReels ? "/content?kind=video&filter=kararli" : "/content?filter=kararli"}
              ...
              return (
                <article key={post.id} className="gg-card" style={{ display: "grid", gap: 10 }}>
                  ...
                  <form action={moderate} style={{ display: "grid", gap: 8 }}>
                    ...
                    <input type="hidden" name="kind" value="gonderi" />
                    <input type="hidden" name="returnTo" value={back} />
