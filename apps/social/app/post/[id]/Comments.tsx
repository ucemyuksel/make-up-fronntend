...
export async function Comments({
  ...
  const all = (await api<Comment[]>(process.env.POST_API, `/api/posts/${postId}/comments`, token ?? "")) ?? [];
  const kokler = all.filter((c) => !c.parentCommentId);
  ...
  async function addComment(formData: FormData) {
    ...
    if (!res || !res.ok) {
      const code = res ? String(res.status) : "baglanti";
      redirect(`/post/${postId}?cerror=${encodeURIComponent(code)}`);
    }
  ...
  const commentForm = (parentId?: string) => (
    <form action={addComment} style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
  ...
  return (
    <section style={{ display: "grid", gap: 14, marginTop: 20 }}>
      <h2 style={{ margin: 0, fontSize: 17 }}>Comments ({all.length})</h2>
        ...
        {kokler.map((k) => (
          <div key={k.id} style={{ display: "grid", gap: 8 }}>
            ...
            <div style={{ display: "grid", gap: 8, marginLeft: 26, paddingLeft: 12, borderLeft: "2px solid var(--gg-border)" }}>
              ...
              {token && !commentsDisabled ? commentForm(k.id) : null}
            </div>
          </div>
        ))}
        {all.length === 0 ? (
