...
export function ShareButton({
  ...
  return (
    <span style={{ position: "relative", display: "inline-flex" }}>
      <button onClick={share} title="Paylaş" aria-label="Paylaş" className="gg-icon-btn">
...
type SavedItem = { id: string; tip: "post" | "reel"; title: string };

function savedItems(): SavedItem[] {
  try { return JSON.parse(localStorage.getItem("gg-kaydedilenler") ?? "[]"); } catch { return []; }
}

export function SaveButton({ id, tip, title }: SavedItem) {
  const [saved, setKayitli] = React.useState(false);
  React.useEffect(() => setKayitli(savedItems().some((k) => k.id === id)), [id]);

  const toggle = () => {
    const mevcut = savedItems();
    const next = saved ? mevcut.filter((k) => k.id !== id) : [...mevcut, { id, tip, title }];
    localStorage.setItem("gg-kaydedilenler", JSON.stringify(next));
    setKayitli(!saved);
  };

  return (
    <button onClick={toggle} title={saved ? "Kaydedilenlerden çıkar" : "Kaydet"}
            aria-label={saved ? "Kaydedilenlerden çıkar" : "Kaydet"}
            aria-pressed={saved}
            className="gg-icon-btn"
            style={{ color: saved ? "var(--gg-primary)" : undefined }}>
      <span aria-hidden="true">🔖</span>
      {saved ? <span className="gg-icon-btn-label">Kaydedildi</span> : null}
    </button>
  );
}
...
export function SavedList() {
  const [list, setList] = React.useState<SavedItem[]>([]);
  React.useEffect(() => setList(savedItems()), []);
  if (list.length === 0) {
    ...
    <div style={{ display: "grid", gap: 10 }}>
      {list.map((k) => (
        <div key={k.id} className="gg-card" style={{ display: "flex", gap: 10, alignItems: "center" }}>
          ...
          <SaveButton id={k.id} tip={k.tip} title={k.title} />
        </div>
      ))}
    </div>
  );
...
export function DislikeButton({ id }: { id: string }) {
  ...
  const toggle = () => {
    ...
    const next = begenmedim ? list.filter((x) => x !== id) : [...list, id];
    localStorage.setItem("gg-dislike", JSON.stringify(next));
    setBegenmedim(!begenmedim);
  };
  return (
    <button onClick={toggle} title="Beğenme"
...
type Slide = { tip: "story"; h: Story } | { tip: "ad"; ad: ServedAd; eventId: string };
...
async function reportAdEvent(campaignId: string, type: "impression" | "click", eventId: string) {
  try {
    await fetch(`/api/ads/${campaignId}/${type}`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ eventId, countryCode: browserCountry() || null }),
      keepalive: true,
    });
  } catch { /* sessiz */ }
}
...
export function StoryBar({ stories }: { stories: Story[] }) {
  ...
  const [ads, setAds] = React.useState<ServedAd[]>([]);
    ...
    <>
      <div style={{ display: "flex", gap: 16, overflowX: "auto", paddingBottom: 4 }}>
        {stories.map((s, i) => {
          ...
          const ic = (
            <span style={{ display: "block", width: "100%", height: "100%", borderRadius: "50%", overflow: "hidden", background: s.color, border: "2px solid #fff" }}>
          ...
          return s.href ? (
            <a key={s.ad + i} href={s.href} style={box} aria-label="Hikaye paylaş">
            ...
            <button key={s.ad + i} onClick={() => setActive(i)} style={box}>
              {ring}
              {etiket}
            </button>
          );
      ...
      {active !== null && <StoryViewer stories={stories} ads={ads} startIndex={active} close={() => setActive(null)} />}
    </>
  );
}

function StoryViewer({ stories, ads, startIndex, close }: { stories: Story[]; ads: ServedAd[]; startIndex: number; close: () => void }) {
  // Slaytlar: her 3 hikayede bir ad araya girer (mevcut ad sayısınca).
  const slides = React.useMemo<Slide[]>(() => {
    const out: Slide[] = [];
    let adIndex = 0;
    const newId = () => (typeof crypto !== "undefined" && crypto.randomUUID ? crypto.randomUUID() : `${Date.now()}-${adIndex}`);
    stories.forEach((h, k) => {
      out.push({ tip: "story", h });
      if ((k + 1) % 3 === 0 && adIndex < ads.length) {
        out.push({ tip: "ad", ad: ads[adIndex++], eventId: newId() });
      }
    });
    // Az hikaye varsa (araya reklam girmediyse) sona bir reklam ekle — sponsorlu slayt hep görünsün.
    if (adIndex === 0 && ads.length > 0) {
      out.push({ tip: "ad", ad: ads[0], eventId: newId() });
    }
    return out;
  }, [stories, ads]);
  ...
  const [i, setI] = React.useState(startSlide);
  ...
  React.useEffect(() => {
    ...
    if (cur && cur.tip === "ad" && !firedRef.current.has(cur.eventId)) {
      ...
      reportAdEvent(cur.ad.campaignId, "impression", cur.eventId);
    }
  }, [i, slides]);

  const cur = slides[i];
  if (!cur) return null;
  const isAd = cur.tip === "ad";
  ...
  const ctaTikla = (ad: ServedAd, e: React.MouseEvent) => {
    ...
    reportAdEvent(ad.campaignId, "click", id);
    if (typeof window !== "undefined") window.open(ad.ctaUrl, "_blank", "noopener");
  };

  const arka = isAd
    ? "#000"
    : `linear-gradient(160deg, ${(cur as { h: Story }).h.color}, var(--gg-primary))`;
  const title = isAd ? (cur as { ad: ServedAd }).ad.advertiserName : (cur as { h: Story }).h.ad;

  return (
    <div onClick={() => (i + 1 < slides.length ? setI(i + 1) : close())}
      ...
      <div style={{ width: "min(420px, 94vw)", aspectRatio: "9/16", borderRadius: 18, overflow: "hidden", position: "relative", background: arka, display: "grid", placeItems: "center" }}>
        ...
        <div style={{ position: "absolute", top: 24, left: 14, display: "flex", gap: 8, alignItems: "center", zIndex: 2 }}>
          ...
          <strong style={{ color: "#fff", fontSize: 13, textShadow: "0 1px 4px rgba(0,0,0,.5)" }}>{title}</strong>
          {isAd ? <span style={{ background: "rgba(0,0,0,.55)", color: "#fff", fontSize: 10, fontWeight: 700, borderRadius: 5, padding: "2px 6px" }}>Sponsorlu</span> : null}
        </div>
        ...
        {isAd ? (
          ...
          <p style={{ color: "#fff", fontSize: 17, fontWeight: 600, padding: "0 26px", textAlign: "center", textShadow: "0 1px 6px rgba(0,0,0,.35)", zIndex: 2 }}>{(cur as { h: Story }).h.text}</p>
        )}
      </div>
    </div>
  );
