...
export default async function StoryComposer({
  ...
  async function paylas(formData: FormData) {
    "use server";
    const s = await auth();
  ...
  return (
    <div style={{ maxWidth: 560, display: "grid", gap: 16 }}>
      ...
      <form action={paylas} className="gg-card" style={{ display: "grid", gap: 12 }}>
        <label style={{ display: "grid", gap: 4, fontSize: 13 }}>
          Görsel / video
          <input id="hikaye-medya" name="mediaUrl" required className="gg-search"
                 placeholder="https://... (yükleyince otomatik dolar)" />
        </label>
        <MediaUpload targetId="hikaye-medya" label="📤 Hikaye görseli yükle" accept="image/*,video/*" />
      ...
      <section>
        <h2 style={{ fontSize: 16, margin: "4px 0 10px" }}>Aktif hikayelerim ({benimkiler.length})</h2>
        {benimkiler.length === 0 ? (
