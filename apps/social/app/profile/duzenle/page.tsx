...
type Ulke = { code: string; name: string; dialCode: string | null; featured: boolean };
...
export default async function ProfilDuzenle({
  ...
  const [profil, ulkeler] = await Promise.all([
    fetch(`${USER_API()}/api/users/me`, {
      headers: { Authorization: `Bearer ${token}` },
      cache: "no-store",
    }).then((r) => (r.ok ? (r.json() as Promise<Profil>) : null)).catch(() => null),
    fetch(`${STORE_API()}/api/geo/countries`, { cache: "no-store" })
      .then((r) => (r.ok ? (r.json() as Promise<Ulke[]>) : []))
      .catch(() => [] as Ulke[]),
  ]);

  const kodlu = ulkeler.filter((u) => u.dialCode);
  const seciliKod = profil?.phoneCountryCode ?? "+90";
  ...
  async function kaydet(formData: FormData) {
    ...
    revalidatePath("/profile/duzenle");
  ...
  return (
    <div style={{ maxWidth: 520, display: "grid", gap: 16 }}>
      ...
      <form action={kaydet} className="gg-card" style={{ display: "grid", gap: 12 }}>
        <label style={lbl}>
          Görünen ad
        ...
        <div style={{ display: "grid", gap: 4 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "170px 1fr", gap: 8 }}>
            <select name="phoneCountryCode" className="gg-search" defaultValue={seciliKod}
            ...
            <input name="phone" className="gg-search" inputMode="tel" maxLength={24}
                   defaultValue={profil?.phone ?? ""} placeholder="532 111 22 33"
                   aria-label="Telefon numarası" />
          </div>
          <span style={{ fontSize: 11.5, color: "var(--gg-muted)" }}>
            {profil?.phoneE164
              ? `Kayıtlı: ${profil.phoneE164}`
        ...
        <label style={lbl}>
          ...
          <input name="avatarColorHex" type="color" className="gg-search"
                 defaultValue={profil?.avatarColorHex ?? "#F6C6D8"}
