...
const percent = (n: number) => (Number(n ?? 0) * 100).toFixed(2) + "%";
...
export default async function Ads({
  ...
  const yayinda = all.filter((c) => c.status === "ACTIVE").length;
  const dailySpend = all.reduce((t, c) => t + Number(c.spentToday ?? 0), 0);
  const totalImpressions = all.reduce((t, c) => t + Number(c.impressions ?? 0), 0);
  ...
  async function moderate(form: FormData) {
    ...
    const returnTo = String(form.get("returnTo") ?? "/ads");
    revalidatePath("/ads");
    redirect(returnTo + (returnTo.includes("?") ? "&" : "?") + (ok ? "ok=1" : "error=1"));
  }

  const back = `/ads${status ? `?status=${status}` : ""}`;
    ...
    <main style={{ maxWidth: 980, margin: "0 auto", padding: 32, display: "grid", gap: 22 }}>
      ...
      <form method="get" style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
        <input name="q" defaultValue={q} className="gg-search" style={{ flex: 1, minWidth: 260 }}
               placeholder="Kullanıcı adı, e-posta, telefon veya kampanya adı ara…" />
        <button className="gg-btn gg-btn-primary" type="submit">Ara</button>
        {q ? <a href="/ads" className="gg-btn gg-btn-ghost">Temizle</a> : null}
      </form>
      {q ? (
        <div style={{ fontSize: 13, color: "var(--gg-muted)" }}>
          &quot;{q}&quot; için {items.length} kampanya bulundu.
        </div>
      ) : null}
      ...
      <section style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12 }}>
        {[
          { l: "Onay bekleyen", v: String(pending), d: "PENDING" },
          { l: "Yayında", v: String(yayinda), d: "ACTIVE" },
          { l: "Bugünkü harcama", v: tl(dailySpend), d: "ACTIVE" },
          { l: "Toplam gösterim", v: totalImpressions.toLocaleString("tr-TR"), d: "" },
        ].map((k) => {
          const selected = status === k.d;
          return (
            <a key={k.l} href={k.d ? `/ads?status=${k.d}` : "/ads"}
               className="gg-card"
               style={{
                 display: "grid", gap: 4, textDecoration: "none", color: "inherit",
                 outline: selected ? "2px solid var(--gg-primary, #C56A7A)" : "none",
               }}>
              ...
              <span style={{ fontSize: 11, color: "var(--gg-primary, #C56A7A)" }}>
                {selected ? "✓ haritada gösteriliyor" : "haritada göster ›"}
              </span>
            </a>
          );
        })}
              ...
              return (
                <div key={c.countryCode}
                  ...
                  <span style={{ height: 8, background: "var(--gg-border, #EEE)", borderRadius: 999, overflow: "hidden" }}>
                    <span style={{ display: "block", height: "100%", width: `${share * 100}%`, background: "var(--gg-primary, #C56A7A)", borderRadius: 999 }} />
                  </span>
      ...
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
        {TABS.map((t) => (
          <a
            key={t.key}
            href={t.key ? `/ads?status=${t.key}` : "/ads"}
              ...
              return (
                <article key={c.id} className="gg-card" style={{ display: "grid", gap: 10 }}>
                  ...
                  <div style={{
                    ...
                    <span>CTR: <strong>{percent(c.ctr)}</strong></span>
