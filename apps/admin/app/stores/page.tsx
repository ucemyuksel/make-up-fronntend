...
export default async function Stores() {
  ...
  if (!s?.accessToken) redirect("/"); if (!s.roles?.includes("ADMIN")) redirect("/forbidden");
  const items = await adminApi<Store[]>(process.env.STORE_API!, "/api/stores/moderation/queue", s.accessToken) ?? [];
  async function approve(form: FormData) { "use server"; const session = await auth() as { accessToken?: string } | null; if (session?.accessToken) await adminPost(process.env.STORE_API!, `/api/stores/${form.get("id")}/approve`, session.accessToken); revalidatePath("/stores"); }
