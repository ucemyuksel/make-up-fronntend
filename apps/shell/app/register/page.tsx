...
export default async function SignUpPage({
  ...
  const isStore = searchParams.kind === "store";
  ...
  async function register(form: FormData) {
    ...
    if (message) {
      redirect(`/register?kind=${searchParams.kind ?? ""}&error=${encodeURIComponent(message)}`);
    }
    redirect("/login?kayit=tamam");
  }
        ...
        <form action={register} style={{ padding: 28, display: "grid", gap: 14 }}>
          <div>
            ...
            <p style={{ margin: 0, opacity: 0.7, fontSize: 14 }}>
              {isStore
                ? "Ürünlerini satmak için mağaza hesabı oluşkind."
          ...
          <button type="submit" className="gg-btn gg-btn-primary" style={{ marginTop: 4 }}>
            {isStore ? "Mağaza hesabı oluşkind" : "Üye ol"}
          </button>

          <p style={{ margin: 0, fontSize: 14, textAlign: "center" }}>
            {isStore ? (
              <a href="/register">Bireysel hesap aç</a>
            ) : (
              <a href="/register?kind=store">Mağaza hesabı açmak istiyorum</a>
            )}
            {" · "}
